...
class ResultsCalculator {
    ...
    static buildResultsObject(basicStats, analytics) {
        return {
            ...
            maxMarks: examState.examData.length * 2,
...
class AIInsights {
    ...
    **SSC CGL Tier I EXAM CONTEXT:**
    This is a practice test for SSC-CGL-Tier-I, conducted by Staff Selection Commission for recruitment to Group B and C posts in Central Government. The actual SSC CGL Tier I exam features 100 questions in 60 minutes with negative marking (-0.50 for wrong answers, +2 for correct). Success requires mastering General Intelligence & Reasoning (25Q), General Awareness (25Q), Quantitative Aptitude (25Q), and English Comprehension (25Q). SSC CGL aspirants typically need 120+ marks out of 200 to qualify for Tier II.
...
class StudyPlanDisplay {
    ...
    static displayWeeklyFocus(studyPlan) {
        ...
        // Generate dynamic weekly focus based on actual results and AI recommendations
        const results = DataManager.getResults();
    ...
    static generateDynamicWeeklyFocus(results, studyPlan) {
        // First priority: Use AI-generated study plan if available and valid
        if (studyPlan && studyPlan.weeklyFocus && Array.isArray(studyPlan.weeklyFocus) && studyPlan.weeklyFocus.length > 0) {
            // Use LLM response directly, including any colors it provides
            return studyPlan.weeklyFocus.map(item => ({
                subject: item.subject || 'Study',
                hours: item.hours || 2,
                priority: item.priority || 'medium',
                color: item.color || this.generateSubjectColor(item.subject || 'Study') // Fallback to dynamic color only if LLM doesn't provide
            }));
        }
        
        // Second priority: Generate from exam results if available
        if (results && results.subjectStats && Object.keys(results.subjectStats).length > 0) {
            return this.generateFromResults(results);
        }
        
        // Third priority: Generate from exam data structure
        return this.generateFromExamData();
    }
    ...
    static generateFromResults(results) {
        ...
        // Scale hours if total exceeds reasonable limit
        if (totalHours > 25) {
    ...
    static generateFromExamData() {
        // Extract subjects directly from loaded exam data
        if (!examState.examData || examState.examData.length === 0) {
            return this.getFallbackWeeklyFocus();
        }
        ...
        const subjectColors = this.generateSubjectColorPalette(subjects);
        const baseHours = Math.max(2, Math.floor(20 / subjects.length));
        
        const weeklyFocus = subjects.map(subject => {
            // Give more hours to subjects with more questions
            const questionCount = subjectCounts[subject];
            const totalQuestions = examState.examData.length;
            const proportion = questionCount / totalQuestions;
            const adjustedHours = Math.max(2, Math.round(baseHours * (1 + proportion)));
            
            return {
                subject: subject,
                hours: adjustedHours,
                priority: proportion > 0.3 ? 'high' : proportion > 0.2 ? 'medium' : 'low',
                color: subjectColors[subject]
            };
        });
    ...
    static generateSubjectColorPalette(subjects) {
        // Professional color palette - no hardcoded subject matching
        const colors = [
        ...
        // Simply assign colors in order - no hardcoded subject type detection
    ...
    static generateSubjectColor(subject) {
        // Get all subjects from exam data for consistent color assignment
    ...
    static extractSubjectsFromExamData() {
        // Extract unique subjects from current exam data
        if (!examState.examData || examState.examData.length === 0) {
            return [];
        }
        ...
        // Return subjects in consistent order (alphabetically sorted)
        return Array.from(subjects).sort();
    }

    static getFallbackWeeklyFocus() {
        return [
            {
