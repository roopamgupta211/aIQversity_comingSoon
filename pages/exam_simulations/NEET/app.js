...
const CONFIG = {
    TIMER_INTERVAL: 1000,
    SCORE_THRESHOLDS: { EXCELLENT: 85, GOOD: 70, AVERAGE: 50 },
...
class ResultsCalculator {
    ...
    static buildResultsObject(basicStats, analytics) {
        ...
        const scorePercentage = Math.max(0, Math.round((basicStats.totalMarks / maxPossibleMarks) * 100));
        return {
...
class ResultsDisplay {
    ...
    static displayHeroSection(results) {
        ...
        if (subtextElement) {
            ...
            const marksText = results.totalMarks >= 0 ? `${results.totalMarks} marks earned` : `${results.totalMarks} marks (negative marking)`;
            subtextElement.textContent = `Attempted ${attemptedQuestions} questions • ${results.correct} correct answers • ${marksText}`;
        }
...
class AIInsights {
    ...
    **NEET EXAM CONTEXT:**
    This is a 20-question practice test for the Indian NEET (National Eligibility cum Entrance Test). The actual NEET exam features 180 questions in 3 hours with negative marking (+4 for correct, -1 for wrong answers), but this demo provides a focused sample. Success requires mastering Physics, Chemistry, and Biology (Botany + Zoology) based on NCERT Class 11-12 syllabus. NEET aspirants typically need 600+ marks out of 720 to secure admission in government medical colleges.
    ...
    **CRITICAL REQUIREMENTS:**
    ...
    static displayError() {
        ...
        if (subtextElement) {
            const marksText = results.totalMarks >= 0 ? `${results.totalMarks} marks earned` : `${results.totalMarks} marks`;
            subtextElement.textContent = `${results.correct}/${results.total} questions mastered • ${marksText} • ${Utils.formatTime(results.duration)} spent learning`;
        }
